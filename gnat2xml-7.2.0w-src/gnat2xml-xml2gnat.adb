...
with Gnat2xml.Ada_Trees.Self_Rep; use Gnat2xml.Ada_Trees.Self_Rep;

procedure Gnat2xml.Xml2gnat is

   Output_Dir : Name_Id := Name_Find (".");
   ...
   procedure Do_Nothing
     (Original_Source_Name, Xml_File_Name, Self_Rep_Name, Regen_Ada_Name, Generated_Ada_Name : String) is null;

   procedure Generate_Ada
     (Original_Source_Name, Xml_File_Name, Self_Rep_Name, Regen_Ada_Name, Generated_Ada_Name : String);
   ...
   procedure Generate_Ada
     (Original_Source_Name, Xml_File_Name, Self_Rep_Name, Regen_Ada_Name, Generated_Ada_Name : String) is
      Tree : constant Ada_Tree := Read_Xml (Xml_File_Name);

      Self_Rep_File, Regen_Ada_File, Generated_Ada_File : Text_IO.File_Type;

   begin
      if not Debug_Mode then
         Put ("Creating \1\n", Self_Rep_Name);
         Create_Path (Containing_Directory (Self_Rep_Name));
         Text_IO.Create (Self_Rep_File, Name => Self_Rep_Name);
         Text_IO.Set_Output (Self_Rep_File);
      end if;
      ...
      if not Debug_Mode then
         ...
         Text_IO.Close (Self_Rep_File);
      end if;

      if not Debug_Mode then
         Put ("Creating \1\n", Regen_Ada_Name);
         Create_Path (Containing_Directory (Regen_Ada_Name));
         Text_IO.Create (Regen_Ada_File, Name => Regen_Ada_Name);
         Text_IO.Set_Output (Regen_Ada_File);
      end if;
      ...
      if not Debug_Mode then
         Text_IO.Set_Output (Text_IO.Standard_Output);
         Text_IO.Close (Regen_Ada_File);
      end if;

      if not Debug_Mode then
         Put ("Creating \1\n", Generated_Ada_Name);
         Create_Path (Containing_Directory (Generated_Ada_Name));
         Text_IO.Create (Generated_Ada_File, Name => Generated_Ada_Name);
         Text_IO.Set_Output (Generated_Ada_File);
      end if;
      ...
      if not Debug_Mode then
         Text_IO.Set_Output (Text_IO.Standard_Output);
         Text_IO.Close (Generated_Ada_File);
      end if;
   end Generate_Ada;
         ...
         if Has_Prefix (Argument (X), Prefix => "-") then
            ...
            declare
               ...
               Generated_Ada_Name : constant String :=
                 Compose
                   (Containing_Directory =>
                      Compose (Get_Name_String (Output_Dir), Generated_Ada_Subdir),
                    Name => Simple_Name (Original_Source_Name));
               --  for example, "./generated_ada/some-package.adb"

            begin
               --  We don't want to overwrite the original source code, and
               --  Generated_Ada_Name is the same name. This program should be run in a
               --  different directory, but just in case it isn't we raise an
               --  exception. Note that in --debug mode, we don't write any
               --  files.

               if not Debug_Mode and then Exists (Generated_Ada_Name) then
                  raise Program_Error with "output file already exists: " & Generated_Ada_Name;
               end if;

               Action (Original_Source_Name, Xml_File_Name, Self_Rep_Name, Regen_Ada_Name, Generated_Ada_Name);
            end;
         end if;
      end loop;
   end Iter_File_Names;
