...
package body Gnat2xml.Ada_Trees.Self_Rep is
   ...
   procedure Put_Ada_Tree (Tree : Ada_Tree) is
      ...
      Has_Attrs : constant Boolean :=
        Kind in A_Compilation_Unit | Def_Names |
          Flat_Expression_Kinds | Flat_Pragma_Kinds |
   ...
   begin
      ...
      if Tree.Subtree_Count = 0 and then
        Asis.Text.Is_Nil (Tree.Sloc) and then
        not Has_Attrs
      then
         return;
      end if;
      ...
      case Kind is
         when Opt_ASIS_Elems =>
            declare
               Qs : Query_List_Ptr := Appropriate_Queries (Kind);
            begin
   ...
   begin
      ...
      Put ("with Asis.Extensions.Flat_Kinds; use Asis.Extensions.Flat_Kinds;\n");
      Put ("with Formatted_Output; use Formatted_Output;\n");
      Put ("with Strings; use Strings;\n\n");
      Put ("with Gnat2xml; use Gnat2xml;\n");
      Put ("with Gnat2xml.Ada_Trees; use Gnat2xml.Ada_Trees;\n");
      Put ("with Gnat2xml.Ada_Trees.Self_Rep; use Gnat2xml.Ada_Trees.Self_Rep;\n");
      Put ("with Gnat2xml.Ada_Trees.Factory; use Gnat2xml.Ada_Trees.Factory;\n\n");
      Put ("procedure \1 is\n", Proc_Name);
      Indent;
      Put ("\nuse all type Unit_Kinds, Unit_Classes, Unit_Origins, Asis.Mode_Kinds;\n");
   ...
   procedure Put_Regen_Ada (Tree : Ada_Tree) is
      Source_File_Name : constant String :=
        "stage/1/" & Get_Name_String (Tree.Source_File);
   begin
      Put ("with GNAT.OS_Lib; use GNAT.OS_Lib;\n");
      Put_Head ("Regenerate_Ada", Tree);
      Indent;
      Put ("Source_File_Name : constant String :=\n\i""\1"";\o\n", Source_File_Name);
      Put ("Source_File_Contents : String_Access :=\n");
      Put ("\iRead_File (Source_File_Name);\n\o");
      Outdent;
      Put ("begin\n");
      Indent;
if False then -- ???Messes up diff's
      ...
      Put ("Tree_To_Ada (The_Tree, Source_File_Contents.all);\n");
      Put ("Free (Source_File_Contents);\n");
      Outdent;
      Put ("end Regenerate_Ada;\n\n");
   end Put_Regen_Ada;

end Gnat2xml.Ada_Trees.Self_Rep;
