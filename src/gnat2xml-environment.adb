...
package body Gnat2xml.Environment is
   ...
   begin
      Put_Line ("Usage: gnat2xml [options] files");
      Put_Line ("");
      Put_Line ("""files"" are the Ada source file names.");
      Put_Line ("");
      Put_Line ("Options:");
      Put_Line ("");
      Put_Line ("    -mdir -- generate one .xml file for each Ada source file, in directory");
      Put_Line ("             'dir'. (Default is to generate the XML to standard output.)");
      Put_Line ("");
      Put_Line ("    -q -- debugging version, with interspersed source, and a more");
      Put_Line ("          compact representation of ""sloc"". Note that this version");
      Put_Line ("          currently does NOT validate.");
      Put_Line ("");
      Put_Line ("    -v -- verbose (print out the names of output files as they are generated).");
      Put_Line ("");
      Put_Line ("    -t -- do not delete tree files when done (they are deleted by default).");
      ...
      Put_Line ("   -q     Generate less verbose XML");
...
--???      Unit_Names.Include (Unit_Name);
      File_Name_Handling.Include (Unit_Names, Unit_Name);
   end Add_Files;
   ...
   procedure Process_Args_And_Options
     (The_Node_Information :    out Gnat2xml.Xml.Info_Node;
      ...
      Option_Table : constant Avatox_Cmd_Line.Command_Line_Item_Mappings :=
        (Avatox_Cmd_Line.Set_Option (Include, 'I',
         ...
         Avatox_Cmd_Line.Set_Option (Output_File, 'o',
           Case_Sensitive  => True,
           Argumented      => True),
         Avatox_Cmd_Line.Set_Option (Compact_XML, "q",
           Case_Sensitive  => True,
           Argumented      => False),
         Avatox_Cmd_Line.Set_Option (Debug_Mode, "g",
           Case_Sensitive  => True,
           Argumented      => False),
      ...
      loop
         ...
         when Avatox_Cmd_Line.CLI_Option =>
            case Item.Option is
               ...
               when Compact_XML =>
raise Program_Error;
--                  Xml.Compact_XML := True;

               when Debug_Mode =>
                  Gnat2xml.Debug_Mode := True;
      ...
      if Unit_Breadth = Gnat2xml.Environment.Full_Closure then   -- -c disable
   ...
   begin
      ...
      while Fc /= No_Element loop
         declare
            ...
            Dir : constant String := Directories.Containing_Directory (File_Spec);
            Simple : constant String := Directories.Simple_Name (File_Spec);
         begin
