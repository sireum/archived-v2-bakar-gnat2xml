...
package Gnat2xml.Ada_Trees.Self_Rep is
   ...
   --  Put_Self_Rep. If compiled and run, the Ada program should print a copy
   --  of its own source code. Used for testing.

   procedure Put_Regen_Ada (Tree : Ada_Tree);
   --  Prints out a program that converts the tree back into Ada code, and
   --  writes it to current output. Writes the program to current output. Used
   --  for testing.
